...
Importantly, \Cref{eq:value_expansion} holds \emph{only} if the value function is correct \citep{sutton2018reinforcement}.
%
...
\Cref{eq:TD-learning} is the canonical temporal difference learning rule, and it leads to learning dynamics where the temporal difference error $\delta := - V(s_t) + r(s_t) + \gamma \mathbb{E}_\pi \left [ V(s_{t+1}) \right ]$ is progressively propagated from the initial rewarding state to prior states that predict the reward.
