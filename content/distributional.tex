...
\begin{itemize}
    ...
    This is consistent with a distributional RL theory, where neurons correspond to the TD updates considered above, since the reversal point of a neuron $i$ should in this case be $\epsilon_{\tau_i}$ (\Cref{fig:dist}B).
    ...
    This is consistent with the expectile distributional RL setting, where the reversal point is $\epsilon_\tau = \frac{\alpha^{+}}{\alpha^- + \alpha^+}$ (\Cref{fig:dist}B).
    \item When `imputing' the distribution (approximated as a set of delta functions) implied by the VTA neurons when interpreted as expectiles (\Cref{fig:dist}C), the resulting fit was remarkably similar to the distribution learned by a simulated distributional RL agent.
\end{itemize}
