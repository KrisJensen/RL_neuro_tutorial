...
\end{equation}
where $R_t$ and $V(s_t)$ are considered constant w.r.t. $\theta$.
...
For example, \citet{li2022integrating} recently demonstrated that an off-policy variant known as the `soft actor-critic' algorithm \citep{haarnoja2018soft} could be used to train an agent that shapes the behavior of \textit{C. elegans} via an optogenetic action space.
