...
An on-policy version of Q-learning known as `SARSA' (state-action-reward-state-action) is also commonly used, where the Q-learning update uses the Q-value corresponding to the action $a_t$ sampled at the next timestep instead of the greedy action (\Cref{fig:Q}C):
\begin{align}
