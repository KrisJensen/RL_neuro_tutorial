\begin{figure*}[!t]
    ...
    \caption[Distributional]{\label{fig:dist}
        ...
        {\bfseries (A)}~Distribution of rewards (black), plotted together with the values $V_{\tau_i}$ converged to across 20 units all learning through standard TD learning (blue; all $\tau_i = 0.5$), or through distributional RL with different $\tau_i = \frac{\alpha_i^+}{\alpha_i^+ + \alpha_i^-}$ (green).
        ...
        {\bfseries (C)}~True reward distribution (black) reproduced from (A), now plotted together with the reward distribution $p_{\{ V_{\tau_i} \} }(\tilde{Z}^\pi(s))$ implied by the distributional units from (A) and (B) at different stages of learning (light to dark green).
